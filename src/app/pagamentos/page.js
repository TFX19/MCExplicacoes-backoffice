...
export default function PagamentosPage() {
  ...
  useEffect(() => {
    ...
    sessoesApi
      .listar({ alunoId: form.alunoId, limit: 50 })
  ...
  async function handleSave() {
    if (!form.alunoId || !form.valor) {
      setError("Aluno e valor são obrigatórios.");
      return;
    }
    setSaving(true);
    setError("");
  ...
  async function handleEliminar(idOrIds) {
    ...
    if (Array.isArray(idOrIds)) {
      if (!confirm(`Eliminar ${idOrIds.length} pagamento(s)? Esta ação não pode ser desfeita.`)) return;
      try {
        await pagamentosApi.eliminarMultiplos(idOrIds);
        load();
      } catch (e) {
      ...
      try {
        await pagamentosApi.eliminar(idOrIds);
        setModal(null);
        load();
      } catch (e) {
        alert(`Erro ao eliminar: ${e.message}`);
  ...
  const cols = [
    { key: "aluno", label: "Aluno", render: (r) => r.aluno?.nome ?? "—" },
    {
      key: "valor",
      label: "Valor",
      render: (r) => `€${Number(r.valor).toFixed(2)}`,
    },
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Aluno" required>
              <Select
                value={form.alunoId}
                onChange={(v) =>
                  setForm((f) => ({ ...f, alunoId: v, sessaoId: "" }))
                }
